...
func TestAssertionEvaluator_ResourceRead(t *testing.T) {
	tests := []struct {
		name          string
		...
		{
			name:          "Resource was read",
			resourcesRead: []string{"file://config.json", "file://data.txt"},
			resourcePath:  "file://config.json",
			wantPassed:    true,
		},
		{
			name:          "Resource was not read",
			resourcesRead: []string{"file://config.json"},
			resourcePath:  "file://other.txt",
			wantPassed:    false,
		},
		{
		...
		{
			name:          "Multiple resources, checking last",
			resourcesRead: []string{"file://a.txt", "file://b.txt", "file://c.txt"},
			resourcePath:  "file://c.txt",
			wantPassed:    true,
		},
		{
			name:          "HTTP resource",
			resourcesRead: []string{"https://api.example.com/data"},
		...
		},
		{
			...
			resourcesRead: []string{"file://config.json"},
			resourcePath:  "file://config.json.bak",
			wantPassed:    false,
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			result := &model.ExecutionResult{
				MCPOperations: model.MCPOperations{
					ResourcesRead: tt.resourcesRead,
				},
			}
...
func TestAssertionEvaluator_FileCreated(t *testing.T) {
	tests := []struct {
		name         string
		filesCreated []string
		filePath     string
	...
	}{
		{
			...
			filesCreated: []string{"/tmp/output.txt", "/tmp/result.json"},
			filePath:     "/tmp/output.txt",
			wantPassed:   true,
		},
		{
			name:         "File was not created",
			filesCreated: []string{"/tmp/output.txt"},
		...
		{
			name:         "Relative path",
			filesCreated: []string{"./output.txt", "./data/result.json"},
			filePath:     "./output.txt",
			wantPassed:   true,
		},
		{
			name:         "Windows path",
			filesCreated: []string{"C:\\Users\\test\\output.txt"},
			filePath:     "C:\\Users\\test\\output.txt",
			wantPassed:   true,
		},
		{
			name:         "Case sensitive",
		...
		{
			name:         "Multiple files with same name in different dirs",
			filesCreated: []string{"/tmp/output.txt", "/home/output.txt"},
			filePath:     "/tmp/output.txt",
			wantPassed:   true,
		},
	}
...
func TestAssertionEvaluator_FileWritten(t *testing.T) {
	tests := []struct {
		name         string
		filesWritten []string
		filePath     string
	...
	}{
		{
			...
			filesWritten: []string{"/tmp/log.txt", "/tmp/data.json"},
			filePath:     "/tmp/log.txt",
			wantPassed:   true,
		},
		{
			name:         "File was not written",
			filesWritten: []string{"/tmp/log.txt"},
			filePath:     "/tmp/other.txt",
			wantPassed:   false,
		},
		{
		...
		{
			name:         "Multiple writes to same file",
			filesWritten: []string{"/tmp/log.txt", "/tmp/log.txt", "/tmp/log.txt"},
			filePath:     "/tmp/log.txt",
			wantPassed:   true,
		},
		{
			name:         "Absolute and relative paths",
			filesWritten: []string{"/home/user/file.txt", "./file.txt"},
...
func TestAssertionEvaluator_FileDeleted(t *testing.T) {
	tests := []struct {
		name         string
		filesDeleted []string
		filePath     string
	...
	}{
		{
			...
			filesDeleted: []string{"/tmp/old.txt", "/tmp/temp.log"},
			filePath:     "/tmp/old.txt",
			wantPassed:   true,
		},
		{
			name:         "File was not deleted",
			filesDeleted: []string{"/tmp/old.txt"},
			filePath:     "/tmp/keep.txt",
			wantPassed:   false,
		},
		{
			name:         "Empty files list",
		...
		},
		{
			...
			filesDeleted: []string{"/tmp/a.txt", "/tmp/b.txt", "/tmp/c.txt"},
			filePath:     "/tmp/b.txt",
			wantPassed:   true,
		},
		{
			name:         "Directory deletion",
			filesDeleted: []string{"/tmp/old_directory"},
			filePath:     "/tmp/old_directory",
			wantPassed:   true,
		},
		{
			name:         "Glob pattern not supported",
			filesDeleted: []string{"/tmp/file1.txt", "/tmp/file2.txt"},
			filePath:     "/tmp/*.txt",
			wantPassed:   false,
		},
	}
...
func TestAssertionEvaluator_MCPOperations_Combined(t *testing.T) {
	t.Run("Multiple operations in one execution", func(t *testing.T) {
		result := &model.ExecutionResult{
			MCPOperations: model.MCPOperations{
				ResourcesRead: []string{"file://config.json", "file://data.txt"},
				FilesCreated:  []string{"/tmp/output.txt", "/tmp/result.json"},
				FilesWritten:  []string{"/tmp/output.txt", "/tmp/log.txt"},
				FilesDeleted:  []string{"/tmp/old.txt"},
			},
		}
		...
		assertions := []model.Assertion{
			{Type: "resource_read", Path: "file://config.json"},
			{Type: "file_created", Path: "/tmp/output.txt"},
			{Type: "file_written", Path: "/tmp/log.txt"},
			{Type: "file_deleted", Path: "/tmp/old.txt"},
		}
		...
		for i, result := range results {
			assert.True(t, result.Passed,
				"Assertion %d (%s) should pass", i, result.Type)
		}
	})
	...
	t.Run("Mixed pass and fail", func(t *testing.T) {
		...
		assertions := []model.Assertion{
			{Type: "resource_read", Path: "file://config.json"},  // pass
			{Type: "resource_read", Path: "file://missing.json"}, // fail
			{Type: "file_created", Path: "/tmp/output.txt"},      // pass
			{Type: "file_created", Path: "/tmp/missing.txt"},     // fail
		}
